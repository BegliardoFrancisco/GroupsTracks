            ...
            async with self.async_session() as session:
                async with session.begin():
                    await session.execute(
                        delete(PlayList_TrackDAO).where(
                            PlayList_TrackDAO.PlaylistId == playlist_id and PlayList_TrackDAO.TrackId == track_id)
                    )
        except Exception as e:
        ...
        try:
            async with self.async_session() as session:
                async with session.begin():
                    await session.execute(
                        delete(PlayList_TrackDAO).where(
                            PlayList_TrackDAO.PlaylistId == playlist_id and PlayList_TrackDAO.TrackId in tracks_id)
                    )
        except Exception as e:
